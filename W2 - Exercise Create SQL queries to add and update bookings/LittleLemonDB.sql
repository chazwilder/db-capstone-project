...
CREATE PROCEDURE AddBooking(IN CHECK_DATE DATE, IN TIMESLOTS TIME, IN N_OfGUESTS INT, IN CUSTOMER_ID INT, IN TABLE_ID INT)
BEGIN 
SET autocommit = 0;
START TRANSACTION;
	...
	IF EXISTS (
		...
		WHERE TABLEID = TABLE_ID
		AND BOOKINGDATE = CHECK_DATE
        AND TIMESLOT = TIMESLOTS
	) THEN
		
		ROLLBACK;
		SELECT CONCAT('Table ',TABLE_ID,' is already booked. Booking Cancelled') as BOOKING_STATUS;
	ELSE 
		INSERT INTO Bookings(BookingDate,TimeSlot,NumberOfGuests,CustomerId,TableID)
		VALUES (CHECK_DATE,TIMESLOTS,N_OfGUESTS,CUSTOMER_ID,TABLE_ID);
		COMMIT;
        SELECT CONCAT('Table ',TABLE_ID,' has successfully been booked. See you soon!') as BOOKING_STATUS;
	END IF;
 END$$
